...
def evaluate_tflite(tflite_path: Path, pairs, visualize=False, num_vis=5, vis_dir: Path | None = None):
    ...
    for idx, (img_path, mask_path) in enumerate(tqdm(pairs, desc='Evaluating')):
        ...
        if visualize and idx < num_vis and vis_dir is not None:
            ...
            plt.subplot(1, 3, 1); plt.title('Image'); plt.imshow(img.numpy()); plt.axis('off')
            plt.subplot(1, 3, 2); plt.title('Ground Truth'); plt.imshow(mask.numpy()[:, :, 0], cmap='gray'); plt.axis('off')
            plt.subplot(1, 3, 3); plt.title('Prediction'); plt.imshow(pred_mask[:, :, 0], cmap='gray'); plt.axis('off')
...
def parse_args():
    p = argparse.ArgumentParser(description='Evaluate a fully-int8 TFLite segmentation model.')
    ...
    p.add_argument('--num_vis', type=int, default=5, help='Number of samples to visualize')
    return p.parse_args()
