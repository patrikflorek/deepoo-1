...
IMG_SIZE = (1000, 1000)
TRAIN_IMG_DIR = Path(__file__).resolve().parent.parent / 'data' / 'normalized' / 'train' / 'images'
...
def parse_args():
    ...
    parser.add_argument("--num_calib_images", type=int, default=100,
                        help="Number of training images to use for post-training integer quantisation calibration")
    parser.add_argument("--output_dir", type=str, default=str(MODEL_DIR / "tflite"),
...
def _representative_dataset_gen(num_samples: int):
    ...
    img_paths = sorted([str(p) for p in TRAIN_IMG_DIR.iterdir() if p.suffix.lower() in ['.png', '.jpg', '.jpeg']])[:num_samples]
    if not img_paths:
        raise RuntimeError(f"No training images found in {TRAIN_IMG_DIR}. Cannot build representative dataset.")
    ...
    def gen():
        for img_path in img_paths:
            ...
            img = tf.cast(img, tf.float32) / 255.0  # model was trained on normalised images
...
def load_model(model_path: Path):
    ...
    try:
        return tf.keras.models.load_model(
            model_path,
            compile=False,
            custom_objects={"DeepLabV3Plus": DeepLabV3Plus}
        )
    except (NotImplementedError, ValueError) as e:
        print("[WARN] Could not load full model (", e, "). Falling back to architecture+weights.")
